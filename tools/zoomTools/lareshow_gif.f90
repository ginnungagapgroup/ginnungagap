...
integer :: colormap(3,0:1), psize

psize=10 ! pix size

if(iargc().lt.2) then
  print*,'Usage: lareshow lare-file ngrid'
  return
endif
...
allocate(pix((msize(1)+2)*psize,(msize(2)+2)*psize))
...
do while(m(1).ne.mend(1))
  !print*, m
  if(grid(m(1),m(2),m(3))) pix(i*psize+1:(i+1)*psize-1,j*psize+1:(j+1)*psize-1)=1
  i=i+1
  call next(1)
enddo
j=j+1
...
call writegif(filein(1:ilast)//'_'//tmpnum(3:5)//'.gif', pix, colormap)
num = num+1
call next(3)
enddo
