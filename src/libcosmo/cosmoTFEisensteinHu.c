/*
   The License of this file is somewhat unclear:  Despite some minor
   adjustements, the code is written by Daniel J. Eisenstein and Wayne
   Hu.  It has been converted to double precision and slightly adapted
   to fit the needs of `ginnungagap'.  Additionally, some tweaks have
   been applied to soothe compiler warnings.  I tried to mark all
   changes with EDIT: and an explanation for why it was edited (keeping
   the original version as a comment).
         Steffen Knollmann, 2010
*/
...
// EDIT: Removing unused variables
...
// EDIT: gcc is warning about SQR:
//   cosmoTFEisensteinHu.c: warning: operation on ‘sqrarg’ may be undefined
// hence replaced with simple multiplications.
